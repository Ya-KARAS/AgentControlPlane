...
1. 用户发送 `@ACP`，并在后面描述工程需求。`@acp` 和
   `@AgentControlPlane` 是等效别名。
2. 脚本读取受限的本机能力摘要，再把这条命令展开成控制提示词，发送给网页 AI。
3. 网页 AI 继续对话，询问缺失信息。
4. 信息齐全后，网页 AI 在 `<ACP_TASK>` 标签之间输出一个 JSON 对象。
5. 脚本校验并暂存任务，状态入口提示等待用户回复“执行”。
   扩展隔离存储会保存十分钟，用于刷新后恢复。替换任务如果改变目标或执行路线，用户需要先
   回复“确认变更”，之后才能回复“执行”。
   右下角胶囊显示当前动作，完整执行路线和变更字段保存在鼠标悬停提示中。
   新的 ACP 提及请求会写入对话级规划屏障。确认和派发会在浏览器级锁内
   重新读取页面最新任务与持久化版本；旧标签页检测到不一致时停止派发。
6. 用户在网页输入框中发送简短确认词。
...
`tests` 保留为兼容字段，表示测试命令数；`test_commands` 明确表达同一统计；
`test_cases` 仅在 ACP 能解析测试框架摘要时返回用例数。`failure_category` 只返回有限的
通用分类，不返回执行器错误原文。工作区字段只包含本机能力摘要已公开的目录别名，不包含
配置根路径。结果不会包含目标、总结、完整路径、文件名、日志、凭据或执行器原始输出。

## 模块边界

- `userscript/src/conversation-protocol.js` 负责解析启动命令、任务信封、确认词和安全结果投影。
- `userscript/src/stage-state.js` 负责带版本的对话作用域、规划屏障、版本匹配、过期和旧标签判断。
...
1. 协议测试覆盖精确提及、有限任务提取、确认词、受限执行选择、稳定任务标识和结果脱敏。
   状态测试覆盖对话隔离、过期、版本冲突、规划屏障和旧标签页观察结果。
2. 静态测试要求原生对话协议标记，并禁止任务表单、网页存储、凭据、原始任务 API 和
   HTML 注入。扩展隔离存储只能保存有期限的受限任务信封。
