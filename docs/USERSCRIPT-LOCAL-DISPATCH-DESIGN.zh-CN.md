...
工作区、执行器、Profile、模型路由、凭据、白名单、速率限制、审计记录和执行器策略均由
本机 ACP 管理。网页任务信封中试图指定这些内容的字段会被丢弃。

## 用户流程

1. 用户发送 `@AgentControlPlane`，并在后面描述工程需求。
2. 脚本把这条命令展开成控制提示词，发送给网页 AI。
3. 网页 AI 继续对话，询问缺失信息。
4. 信息齐全后，网页 AI 在 `<ACP_TASK>` 标签之间输出一个 JSON 对象。
5. 脚本校验并暂存任务，状态入口提示等待用户回复“执行”。
6. 用户在网页输入框中发送简短确认词。
7. ACP 创建候选。本机设置决定是使用已保存选择直接派发，还是打开一次性审核页。
...
候选不包含工作区路径、执行器、Profile、模型、凭据、完整对话、浏览器标识或本机文件内容。
