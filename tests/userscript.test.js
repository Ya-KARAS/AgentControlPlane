...
test("userscript declares the natural-language bridge metadata and supported sites", () => {
  ...
  assert.match(script, /^\/\/ @version\s+0\.8\.2$/m);
  assert.match(script, /^\/\/ @name:zh-CN\s+AgentControlPlane 网页桥接预览$/m);
  assert.match(script, /^\/\/ @downloadURL\s+https:\/\/raw\.githubusercontent\.com\/Ya-KARAS\/AgentControlPlane\/refs\/heads\/main\/userscript\/releases\/0\.8\.2\/agent-control-plane-web-bridge\.user\.js$/m);
...
test("userscript update metadata is small and matches the install header", () => {
  ...
  assert.match(meta, /^\/\/ @version\s+0\.8\.2$/m);
  assert.match(meta, /userscript\/releases\/0\.8\.2\/agent-control-plane-web-bridge\.user\.js/);
